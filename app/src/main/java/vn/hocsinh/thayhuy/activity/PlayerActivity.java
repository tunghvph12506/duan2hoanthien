...
import vn.hocsinh.thayhuy.dialogs.AudienceDialog;
import vn.hocsinh.thayhuy.dialogs.CallDialog;
import vn.hocsinh.thayhuy.dialogs.NoticeDialog;
import vn.hocsinh.thayhuy.dialogs.ScoreDialog;
import vn.hocsinh.thayhuy.layout.MoneyLayout;
import vn.hocsinh.thayhuy.manager.DatabaseManager;
import vn.hocsinh.thayhuy.manager.MusicManager;
import vn.hocsinh.thayhuy.model.Question;
...
public class PlayerActivity extends Activity implements View.OnClickListener {
    ...
    private void loadRules() {
        ...
        App.getMusicPlayer().play(MusicManager.LUAT_CHOI, mp -> {
            App.getMusicPlayer().stop();
            noticeDialog.setCancelable(false);
            noticeDialog.setNotification("Bạn đã sẵn sàng chơi với chúng tôi ?", "Sẵn sàng", "Bỏ qua", v -> {
                switch (v.getId()) {
    ...
    private void startGame() {
        ...
        App.getMusicPlayer().play(R.raw.gofind, mp -> {
            ...
            drawerLayout.openDrawer(GravityCompat.START);
            App.getMusicPlayer().play(MusicManager.QUEST_1, mp1 -> {
                App.getMusicPlayer().stop();
                if (mp1 == null) {
                    handler.postDelayed(() -> playGame(), 3000);
                } else {
                    playGame();
                }
            });
    ...
    private void getNewQuestion() {
        ...
        layoutMoney.setBackGroundLevel(level);
        drawerLayout.openDrawer(GravityCompat.START);
        App.getMusicPlayer().play(App.getMusicPlayer().getIdsRaw(level), mp -> {
            App.getMusicPlayer().stop();
            if (mp == null) {
                handler.postDelayed(() -> {
                    drawerLayout.closeDrawer(GravityCompat.START);
    ...
    private void checkAnswer(final View v, int[] ans, final int[] trueAns, final int id) {
        ...
        App.getMusicPlayer().play(ans, mp -> {
            App.getMusicPlayer().stop();
            if (mp == null) {
                handler.postDelayed(() -> {
                    if (getTrueAnswer() == id) {
                ...
                if (getTrueAnswer() == id) {
                    if (level == 5 || level == 10 || level == 15) {
                        App.getMusicPlayer().play(MusicManager.ANS_NOW, mp1 -> {
                            App.getMusicPlayer().stop();
                            answerTrue(v, trueAns);
                        });
                    } else {
    ...
    private void answerFalse(final View v, int[] loseAnswer) {
        ...
        App.getMusicPlayer().play(loseAnswer, mp -> {
            App.getMusicPlayer().stop();
            if (mp == null) {
                handler.postDelayed(() -> saveScore(false), 3000);
            } else {
                saveScore(false);
            }
        });
    ...
    private void answerTrue(final View v, int[] trueAnswer) {
        ...
        App.getMusicPlayer().play(trueAnswer, mp -> {
            App.getMusicPlayer().stop();
            if (mp == null) {
    ...
    public void onClick(final View v) {
        switch (v.getId()) {
            ...
            case R.id.btn_change:
                noticeDialog.setNotification("Bạn thực sự muốn đổi câu hỏi ?", "Đồng ý", "Hủy bỏ", v1 -> {
                    if (v1.getId() == R.id.btn_ok) {
            ...
            case R.id.btn_5050:
                ...
                App.getMusicPlayer().play(MusicManager.SOUND_5050, mp -> {
                    App.getMusicPlayer().stop();
                    App.getMusicPlayer().resumeBgMusic();
                    int count = 0;
                    int b = 0;
            ...
            case R.id.btn_call:
                ...
                App.getMusicPlayer().play(R.raw.help_call, mp -> {
                    App.getMusicPlayer().stop();
                    callDialog.show();
            ...
            case R.id.btn_audience:
                ...
                audienceDialog.show();
                App.getMusicPlayer().play(R.raw.khan_gia, mp -> {
                    App.getMusicPlayer().stop();
    ...
    public void saveScore(boolean stopGame) {
        ...
        if (level > 1) {
            ...
            scoreDialog.setOnDismissListener(dialog -> App.getMusicPlayer().play(MusicManager.LOSE, mp -> {
                App.getMusicPlayer().stop();
                finish();
            ...
            App.getMusicPlayer().play(MusicManager.LOSE, mp -> {
                App.getMusicPlayer().stop();
                finish();
                App.getMusicPlayer().playBgMusic(R.raw.bgmusic);
            });
    ...
    public void stopGame() {
        ...
        noticeDialog.setNotification("Bạn thực sự muốn dừng cuộc chơi ?", "Đồng ý", "Hủy bỏ", v -> {
